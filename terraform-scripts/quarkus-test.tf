...
resource "github_repository_file" "sonar_project_properties" {
  ...
  content    = replace(replace(file("sonarcloud/sonar_project_properties.txt"), "$${local.organisation}", local.organisation),
    "$${github_repository.quarkus_test.name}", github_repository.quarkus_test.name)
...
resource "github_repository_file" "workflow_build" {
  ...
  content             = file("sonarcloud/workflow_build.txt")
  overwrite_on_create = true
}